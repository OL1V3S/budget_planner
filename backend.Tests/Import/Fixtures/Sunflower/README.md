...
`ParserSpecificPdfFixtures` generates image-only, active-content, and parser-boundary inputs for the contained PdfPig worker. They remain synthetic, in-memory, and free of real statement data.
...
The following later-pipeline cases remain deferred; parser-boundary cases are now generated by `ParserSpecificPdfFixtures`:
